// Mock data for Compte Gestion prototype
// French personal + coloc budget app

window.MOCK = (() => {
  const today = new Date(2026, 4, 12); // 12 mai 2026
  ...
  const pro = {
    ...
    invoices: [
      { id: 1, client: 'La Roche SAS', amount: 1200, status: 'payée', date: '2026-05-04' },
      { id: 2, client: 'Studio Mer', amount: 1800, status: 'payée', date: '2026-04-28' },
      { id: 3, client: 'Pixelmatic', amount: 1200, status: 'en attente', date: '2026-05-10' },
    ],
  };

  // Monthly summary aggregates (May 2026)
  const incomes = [
  ...
  // Monthly totals for current user (Lucas)
  const monthly = {
    incomes: 3500,
    charges_perso: 90.29,
    charges_coloc: 636,
    savings: 400,
    purchases_imputed: 370, // 133 + 135 + 64 + 38
  ...
  // 12-month yearly trend (in EUR for "épargne nette" — final - initial)
  const yearly = [
  ...
  // 30-day account balance curve (Compte courant)
  function buildBalanceCurve() {
    const start = 2840.50;
    const events = [
      { day: 3, delta: -24 },     // assurance
      { day: 4, delta: -133 },    // casque
      { day: 5, delta: -440 },    // loyer (joint refund chez Lucas)
      { day: 6, delta: -42.40 },  // mutuelle
      { day: 8, delta: -32 },     // élec
      { day: 8, delta: -64 },     // resto
      { day: 10, delta: -38 },    // plante
      { day: 12, delta: -13.33 }, // internet
      { day: 14, delta: -29.90 }, // salle
      { day: 15, delta: 640 },    // freelance
      { day: 16, delta: -135 },   // vélo
      { day: 20, delta: -16 },    // eau
      { day: 22, delta: -17.99 }, // netflix
      { day: 27, delta: 2860 },   // salaire
      { day: 27, delta: -400 },   // épargne
      { day: 28, delta: -126.67 },// courses
    ];
    const days = [];
    let bal = start;
    for (let d = 1; d <= 31; d++) {
  ...
  return {
    today, me, users, accounts, incomes, charges, savings, purchases,
    monthly, coloc, yearly, balance_curve: buildBalanceCurve(),
    calendar_events, shopping, pro,
  };
})();
